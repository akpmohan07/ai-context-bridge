...
Architecture reference and contribution guide for AI Context Bridge.
...
- `npm run zip` / `npm run zip:firefox` — build + zip for Chrome/Edge, or
  Firefox (also produces the sources zip AMO requires for review)
- `npm run submit:init` / `npm run submit` — local equivalents of the
  `release` GitHub Actions workflow's store-submission step (Chrome Web
  Store, Edge Add-ons, Firefox Add-ons). See `.claude/skills/release-extension/`
